...
//        }
    ...
    public static boolean checkException(double x) {
        try {
            ...
            if (x <= 0.0) {
                ...
                String msg2 = String.format("Invalid input value. Value must be positive and greater than 0.", x);
                throw new IllegalArgumentException(msg1 + '\n' + msg2);
            }
            if (x == Math.PI || x == Math.E) {
                throw new IllegalArgumentException('\n' + "The system couldn't calculate complex number, only rational number");
            }
            if ((x >= 171.624) && (x < 1.8 * Math.pow(10, 308))) {
                throw new IllegalArgumentException('\n' + "The input a big number that the system couldn't handle");
            }
            if (x >= 1.8 * Math.pow(10, 308)) {
                throw new IllegalArgumentException('\n' + "The input value is too large that has out of the machine memory");
            }

        } catch (NullPointerException e) {
            System.out.println("Not valid input");
            System.out.println("Please re-run this program and input a valid number. Thank you.");
            System.exit(0);
        }
        return true;
    }
    ...
    /**
     * The function checks if user has input a valid number, not String or anything else
     *
     * @return boolean true or false answer for the result
     */
    public static boolean isNumber(String s) {

        try {
            String checkType = new BigDecimal(s).toString();
        } catch (Exception e) {
            return false;
        }
        return true;
    ...
    public static double[] input() {
        ...
        boolean xValid = isNumber(inputX);
        boolean yValid = isNumber(inputY);
    ...
    public static void main(String[] args) {
        System.out.println("Welcome to Beta Function Calculator");
        System.out.println("Please enter x value and y value: ");
